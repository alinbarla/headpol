...
import { getContextSnapshotAction } from "@/app/admin/assistant/actions";
import {
  FilePreviewCard,
...
export function AssistantChatInput({
  ...
  const [loadingKind, setLoadingKind] = useState<AssistantContextKind | null>(
    null
  );
  ...
  async function addContextData(kind: AssistantContextKind) {
    setLoadingKind(kind);
    try {
    ...
    <div className="relative mx-auto w-full max-w-2xl">
      <div
        className={cn(
          ...
          "max-md:rounded-b-none max-md:border-x-0 max-md:border-b-0 max-md:shadow-none",
      ...
      >
          ...
          <div className="flex w-full items-center gap-1">
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <button
                  type="button"
                  disabled={pending || loadingKind !== null}
                  aria-label="Attach admin data"
                ...
                >
                  <PaperclipIcon
                    className={cn(
                      "size-5",
                      loadingKind !== null && "animate-pulse"
                    )}
                  />
                </button>
              </DropdownMenuTrigger>
              <DropdownMenuContent align="start" side="top" className="min-w-48">
                {ASSISTANT_CONTEXT_KINDS.map((kind) => (
                  <DropdownMenuItem
                    ...
                    onSelect={() => void addContextData(kind)}
                  >
                    {loadingKind === kind
                      ? `Loading ${ASSISTANT_CONTEXT_LABELS[kind]}…`
                      : ASSISTANT_CONTEXT_LABELS[kind]}
                  </DropdownMenuItem>
                ))}
              </DropdownMenuContent>
            </DropdownMenu>
        ...
        {dragging ? (
          <div className="pointer-events-none absolute inset-0 z-20 flex flex-col items-center justify-center rounded-2xl border-2 border-dashed border-primary bg-muted/90 max-md:rounded-b-none">
      ...
      <p className="mt-3 hidden text-center text-xs text-muted-foreground md:block">
        Answers can be wrong. Check attached data before acting.
      </p>
    </div>
  );
}
