...
const csp = [
  ...
  `img-src 'self' blob: data: ${googleTag.img} https://i.ytimg.com https://img.youtube.com`,
  "media-src 'self'",
  "font-src 'self'",
  `connect-src 'self' ${googleTag.connect}`,
  `frame-src 'self' ${googleTag.frame} ${publicSiteOrigin} https://www.youtube-nocookie.com https://www.youtube.com${isDev ? ' http://localhost:3000 http://127.0.0.1:3000' : ''}`,
  "object-src 'none'",
  "base-uri 'self'",
  "form-action 'self'",
